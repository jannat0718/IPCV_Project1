...
def mark_pts(im, pts, marker_color = (255, 50, 0), text_color = (0, 50, 255), marker_type = cv2.MARKER_CROSS):
    for i, p in enumerate(pts):
        cv2.drawMarker(im, (int(p[0]), int(p[1])), marker_color, marker_type, 30, 3)
        cv2.putText(im, str(i), (int(p[0]), int(p[1] - 10)), cv2.FONT_HERSHEY_COMPLEX, 2, text_color, 3)
    return im
...
obj_pt_np = np.float32(obj_pts).reshape(2, len(obj_pts)).transpose()
scene_pt_np = np.float32(scene_pts).reshape(2, len(scene_pts)).transpose()

h, status = cv2.findHomography(np.array(obj_pts), np.array(scene_pts), cv2.RANSAC)
perspective_transform = cv2.getPerspectiveTransform(obj_pt_np, scene_pt_np)  
...
transformed_ad_points = perspective_tx(get_advert_coords_from_midpt(ad_im, mp), h)
